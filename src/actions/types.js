export const LOGIN_USER = "LOGIN_USER";
export const LOGIN_USER_SUCCESS = "LOGIN_USER_SUCCESS";
export const LOGIN_USER_FAIL = "LOGIN_USER_FAIL";

export const SIGNUP_USER = "SIGNUP_USER";
export const SIGNUP_USER_SUCCESS = "SIGNUP_USER_SUCCESS";
export const SIGNUP_USER_FAIL = "SIGNUP_USER_FAIL";

export const LOGOUT_USER = "LOGOUT_USER";
export const LOGOUT_USER_SUCCESS = "LOGOUT_USER_SUCCESS";
export const LOGOUT_USER_FAIL = "LOGOUT_USER_FAIL";

export const SAVE_PROFILE = "SAVE_PROFILE";

export const TOKEN_CHANGED = "TOKEN_CHANGED";

export const PROFIL_INFO = "PROFIL_INFO";

export const FETCH_COMPANIES = "FETCH_COMPANIES";
export const FETCH_COMPANIES_SUCCESS = "FETCH_COMPANIES_SUCCESS";
export const FETCH_COMPANIES_FAIL = "FETCH_COMPANIES_FAIL";
export const ADD_COMPANY = "ADD_COMPANY";
export const ADD_COMPANY_SUCCESS = "ADD_COMPANY_SUCCESS";
export const ADD_COMPANY_FAIL = "ADD_COMPANY_FAIL";

export const SELECT_COMPANY = "SELECT_COMPANY";

export const FETCH_PRODUCTS = "FETCH_PRODUCTS";
export const FETCH_PRODUCTS_SUCCESS = "FETCH_PRODUCTS_SUCCESS";
export const FETCH_PRODUCTS_FAIL = "FETCH_PRODUCTS_FAIL";

export const FETCH_USERS = "FETCH_USERS";
export const FETCH_USERS_SUCCESS = "FETCH_USERS_SUCCESS";
export const FETCH_USERS_FAIL = "FETCH_USERS_FAIL";

export const FETCH_USER_STATS = "FETCH_USER_STATS";
export const FETCH_USER_STATS_SUCCESS = "FETCH_USER_STATS_SUCCESS";
export const FETCH_USER_STATS_FAIL = "FETCH_USER_STATS_FAIL";

export const FETCH_PRODUCT_STATS = "FETCH_PRODUCT_STATS";
export const FETCH_PRODUCT_STATS_SUCCESS = "FETCH_PRODUCT_STATS_SUCCESS";
export const FETCH_PRODUCT_STATS_FAIL = "FETCH_PRODUCT_STATS_FAIL";
export const FETCH_COMPANY_STATS = "FETCH_COMPANY_STATS";
export const FETCH_COMPANY_STATS_SUCCESS = "FETCH_COMPANY_STATS_SUCCESS";
export const FETCH_COMPANY_STATS_FAIL = "FETCH_COMPANY_STATS_FAIL";

export const FETCH_ADMINS = "FETCH_ADMINS";
export const FETCH_ADMINS_SUCCESS = "FETCH_ADMINS_SUCCESS";
export const FETCH_ADMINS_FAIL = "FETCH_ADMINS_FAIL";

export const FETCH_SELLERS = "FETCH_SELLERS";
export const FETCH_SELLERS_SUCCESS = "FETCH_SELLERS_SUCCESS";
export const FETCH_SELLERS_FAIL = "FETCH_SELLERS_FAIL";

export const FETCH_DELIVERIES = "FETCH_DELIVERIES";
export const FETCH_DELIVERIES_SUCCESS = "FETCH_DELIVERIES_SUCCESS";
export const FETCH_DELIVERIES_FAIL = "FETCH_DELIVERIES_FAIL";

export const FETCH_ORDERS = "FETCH_ORDERS";
export const FETCH_ORDERS_SUCCESS = "FETCH_ORDERS_SUCCESS";
export const FETCH_ORDERS_FAIL = "FETCH_ORDERS_FAIL";

export const FETCH_REPORTS = "FETCH_REPORTS";
export const FETCH_REPORTS_SUCCESS = "FETCH_REPORTS_SUCCESS";
export const FETCH_REPORTS_FAIL = "FETCH_REPORTS_FAIL";

export const DELIVERY_STATS = "DELIVERY_STATS";
export const DELIVERY_STATS_SUCCESS = "DELIVERY_STATS_SUCCESS";
export const DELIVERY_STATS_FAIL = "DELIVERY_STATS_FAIL";
export const ADMIN_DELIVERY_STATS_SUCCESS = "ADMIN_DELIVERY_STATS_SUCCESS";
